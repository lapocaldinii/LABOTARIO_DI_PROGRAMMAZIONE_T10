...
std::shared_ptr<Chat> ChatRegistry::getOrCreateChat(
        const std::shared_ptr<User>& u1,
        const std::shared_ptr<User>& u2)
{
    if (!u1 || !u2)
        throw std::invalid_argument("Users must be non-null");

    if (u1->id() == u2->id())
        throw std::invalid_argument("Cannot create chat with same user twice");
...
std::shared_ptr<Chat> ChatRegistry::findChat(
        const std::shared_ptr<User>& u1,
