...
void Chat::sendMessage(std::shared_ptr<User> sender, const std::string& text) {
    ...
    m_messages.push_back(std::move(msg));   // NO lock
}

std::vector<Message> Chat::messages() const {
    return m_messages;  // semplice copia
}
...
bool Chat::involves(const std::shared_ptr<User>& u1,
...
std::pair<std::shared_ptr<User>, std::shared_ptr<User>>
Chat::participants() const noexcept {
